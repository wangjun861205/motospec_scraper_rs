...
async fn main() -> Result<()> {
    let store = Arc::new(Mutex::new(MongoStore::new("motospec", "spec")?));
    ...
    let brands = logger.clone().lock().unwrap().get_brand_errors()?;
    for (brand, id) in brands {
        retry_scrape_models(client.clone(), brand, id, store.clone(), logger.clone()).await;
    }
    let models = logger.clone().lock().unwrap().get_model_errors()?;
